import { EnvelopeIcon, EnvelopeOpenIcon, FileMd, FilePy, Folder, FolderOpen } from "@phosphor-icons/react";
...
export default function DesktopIcon(){
  const { pathname } = useLocation();
    return (
        <>
            <div className="flex flex-row flex-wrap md:flex-col desktopIcon col py-8 px-4 w-fit gap-8">
                <Link to='/projects'>
                ...
                <Link to='/sent-msg'>
                    <div className="desktopIcon__resume flex flex-col items-center">
                        {
                            pathname==='/sent-msg' ? <EnvelopeOpenIcon className='text-white' size={32} weight="duotone" /> : <EnvelopeIcon className='text' size={32} weight="duotone" />
                        }
                        <p className={`${pathname=='/sent-msg'?'text-white text':'text'}`}>SendMsg.txt</p>
                    </div>
                </Link>
            </div>
        </>
